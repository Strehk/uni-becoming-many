  ...
  return await page.evaluate(async (durationMs: number) => {
    ...
    const renderer = dbg?.renderer as
      | (import("three/webgpu").WebGPURenderer & {
          info: { render: { drawCalls?: number; calls?: number; triangles: number }; memory: { geometries: number; textures: number } };
        })
      | undefined;
    if (!renderer) return { gpuMs: -1, frameMs: -1, drawCalls: 0, triangles: 0, geometries: 0, textures: 0 };
    const gpu: number[] = [];
    const frame: number[] = [];
    ...
    while (performance.now() < end) {
      ...
      last = now;
      try {
        const t = await (renderer as unknown as { resolveTimestampsAsync(): Promise<number> }).resolveTimestampsAsync();
...
async function solo(senseLabel: string): Promise<void> {
  await page.evaluate((label: string) => {
    ...
    const soloBtn = card && [...card.querySelectorAll("button")].find((b) => b.textContent?.includes("Solo"));
    soloBtn?.click();
  }, senseLabel);
...
const senses = ["Farben", "Echo", "Infrarot", "UV", "Duft", "Netzwerk", "Motion", "Magnetfeld", "Rundum"];
...
for (const [k, v] of Object.entries(rows)) {
  ...
  console.log(`  ${pad(k, 18)} ${padl((d >= 0 ? "+" : "") + d.toFixed(2), 7)} ms  (${v.drawCalls - base.drawCalls > 0 ? "+" : ""}${v.drawCalls - base.drawCalls} calls)`);
}
