...
- Click **"Desktop starten"** on the start menu (the list is `.bm-menu__list button`; the other modes are "Mobil"/"ICAROS"), wait ~2 s, then press **Enter** (start gate: "Knopf am Controller drücken …"), then wait ~6 s for renderer init + chunk streaming + flora GLBs.
