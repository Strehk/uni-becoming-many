...
- Launch args: `--enable-unsafe-webgpu --use-angle=metal`, `ignoreHTTPSErrors: true` (dev server is HTTPS with a self-signed cert).
- Click **"Experience starten"** on the start menu, then wait ~6 s for renderer init + chunk streaming + flora GLBs.
- **Senses** (world is void-black until one is active): digit keys toggle layers — 2 echo, 3 motion, 4 infrarot, 5 uv, 6 farben, 7 duft, 8 magnetfeld, 9 netzwerk, 0 rundum. Digit1 clears all. Manual key press flips `senseAuthority` to "manual", so the Theatre timeline stops overwriting it.
